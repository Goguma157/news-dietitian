                    ...
                    if res:
                        ...
                        with st.form(key=f"chat_form_{i}"):
                            ...
                            if submit_btn and user_q:
                                ...
                                # AI 답변 생성
                                with st.spinner("AI가 생각 중..."):import streamlit as st
import feedparser
from groq import Groq
import json
import requests
...
# 🚨 [핵심 변경] Google 뉴스 RSS 주소로 전면 교체 (정확도 100%)
rss_feeds = {
...
try:
    # Google 뉴스는 User-Agent가 없으면 차단될 수 있으므로 헤더 추가
...
if news and news.entries:
    ...
    for i, entry in enumerate(news.entries[:10]):
        with cols[i % 2]:
            with st.container(border=True):
                # 구글 뉴스는 제목에 언론사 이름이 포함됨 (예: "제목 - 연합뉴스")
                ...
                if st.button("✨ 영양 성분 분석", key=f"btn_{i}", use_container_width=True, type="primary"):
                    with st.spinner("AI가 분석 중..."):
                        # 구글 뉴스는 description이 짧거나 HTML일 수 있어 제목 위주로 분석
                        res = analyze_news_groq(f"제목: {clean_title}\n내용: {entry.title}에 대한 뉴스입니다.")
                        ...
                        with st.form(key=f"chat_form_{i}"):
                            ...
                            if st.form_submit_button("전송"):
                                ...
                                with st.spinner("답변 중..."):
                                    ai_answer = ask_ai_about_news(clean_title, user_q)
